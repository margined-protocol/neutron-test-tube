...
func SetupNeutronApp() (*app.NeutronApp, []byte) {
	db := dbm.NewMemDB()
	encCfg := app.MakeEncodingConfig()

	appInstance := app.NewNeutronApp(
		log.NewNopLogger(),
		db,
		nil,
		true,
...
func (env *TestEnv) beginNewBlockWithProposer(proposer sdk.ConsAddress, timeIncreaseSeconds uint64) {
	...
	header := tmproto.Header{ChainID: "neutron-666", Height: env.Ctx.BlockHeight() + 1, Time: newBlockTime}
